...
540 conditions were evaluated as a full factorial, with
observer-parameter ranges propagated into 95% bands.

**Results.** The delivered band reaches 0.335 lp/mm in the
median, 26% of the reconstruction Nyquist
frequency, and $R_{\mathrm{perceptual}}$ is 0.226. Neural noise
supplies 63% of the effective noise, and all
108 dose series show declining $G_{\mathrm{useful}}$. In a U-HRCT
case study the band above the conventional Nyquist frequency adds
0.33% of $d'^2$ in the median; its largest share,
12.3% for the 0.5 mm task, falls
where absolute detectability is lowest ($d'=0.36$).
Against human observer data admitted by criteria frozen before the literature
search, the model reproduces the within-study ordering in 2 of
3 studies (pooled $\rho=+0.724$, threshold
0.7); the discordant study is retained.
...
detectability integral to 0.0e+00 relative spread, confirming
invertible-filter invariance and identifying the kernel sensitivity observed with
the floors on (8.5% median spread) as the footprint of
the neural noise (Figure 1). NPWE [@burgess1994] and channelised Hotelling
[@myers1987] observers rank the conditions
alike (Spearman $\rho$ = 1.00).

![**Figure 1.** Left: $d'^2$ across the three reconstruction kernels, relative to the
...
21, 28 and 8 condition points.
Within-study Spearman correlations between
predicted $d'$ and the reported figure of merit are +0.853,
+0.626 and +0.810, against the success threshold of
0.7 fixed in advance. 2 of 3 studies
meet it, and the pooled calibration, formed by standardising ranks within each
study before combining, is +0.724. **H2 is therefore not rejected**:
...
the pool holds two studies and calibrates at +0.841; the
intermediate reading gives +0.841; the full reading admits the third
study and gives +0.724. All three agree in direction, so the widening
did not manufacture the result — but it did not improve it either, and the
difference between +0.841 and +0.724 is one study
entering.

That study is the discordant one. Paul *et al* fall below threshold at
+0.626 and are retained, because the pre-registration requires a
...
Across the three studies the within-study correlations run from +0.626 to
+0.853 with a median of +0.810. No meta-analytic pooled estimate
...
+0.739 for the former against +0.810 for the latter.
...
out of frequencies to deliver (Figure 2). Across the 540 conditions,
$f_{\mathrm{sat}}(95\%)$ — the frequency below which 95% of $d'^2$ has already
accumulated — spans 0.213 to 0.653 lp/mm with a median
of 0.335 lp/mm (Figure 3). Against a reconstruction Nyquist frequency of
1.28 lp/mm, the median condition delivers detection information over
26% of the band the reconstruction supports. The
...
![**Figure 2.** Contribution density to $d'^2$ against spatial frequency at three dose
...
![**Figure 3.** $f_{\mathrm{sat}}(95\%)$ relative to the reconstruction Nyquist frequency.
...
Display quantisation never exceeds 0.031% in any condition.
...
declines monotonically in 100% of the
108 dose series at the point estimate (Figure 4). The direction is not in
...
![**Figure 4.** $G_{\mathrm{useful}}$ against relative dose with 95% bands, for the standard
...
100% of the series satisfy the rule declared in advance,
that the 95% band on the decline excludes zero. The bands are not narrow: the
$f_{\mathrm{sat}}$ band is 0.125 lp/mm wide in the median
against a median centre of 0.444 lp/mm, or
28% of its own centre, and $R_{\mathrm{perceptual}}$
ranges from 0.045 to 0.319 across all
...
the conventional Nyquist frequency of 1.28 lp/mm
(Figure 5).

![**Figure 5.** Share of $d'^2$ arriving from above the conventional Nyquist frequency of
1.28 lp/mm, over anatomical magnification and viewing
distance.](figures/fig5_added_band_map.png)

Across the case study that band contributes 0.33% of
$d'^2$ in the median (Figure 6). The maximum is 12.3%, and where it
occurs is the finding rather than the number itself. It belongs to the
0.5 mm task, whose added-band share is
12.3% and which gains
20.8% in $d'$ from the finer chain — and which sits
at an absolute $d'$ of 0.36. A detectability of that size is
not a detection. The
...
The 8 mm task makes the same point from the other end. It is
detected comfortably, at $d'$ = 12.31, and it takes
1.4% from the finer chain with an added-band share
of 0.2%. Where detection is happening, the added band
...
![**Figure 6.** Maximum added-band share against task diameter, with the absolute $d'$ of the
...
first covers the task, $M^{*}$, is 0.75 for the
conventional chain against 0.50 for the U-HRCT chain in the
...
26% of the reconstruction Nyquist frequency, the
...
0.33% of $d'^2$. These are statements about what reaches
...
before the search, the model orders conditions correctly in 2 of
3 studies, pooling at +0.724. That is agreement in
...
the reported figure of merit. A study succeeds at $\rho \ge$ 0.7. The
...
0.7. H2 is rejected when a majority fall below the threshold, or when
...
documents are in a public repository, released as v0.1.1
(commit `47f9631`) and archived under a DOI. That is the version DOI
...
search is checkable rather than asserted. Commit `6b77ee3` froze
the criteria; commit `21d9421` recorded the first candidate scan.
A reader can confirm the order in the released repository, once its URL is
disclosed at acceptance, with

```
git merge-base --is-ancestor 6b77ee3 21d9421
```
...
**Figure 1.** Left: $d'^2$ across the three reconstruction kernels, relative to the standard kernel, with both noise floors switched off and switched on. With the floors off the kernel leaves the integral exactly, as an invertible filter must. Right: the mechanism — the kernel filters signal and image noise together and leaves the neural floor untouched, so all it can move is which of the two dominates at a given frequency.

**Figure 2.** Contribution density to $d'^2$ against spatial frequency at three dose levels, with $f_{\mathrm{sat}}(95\%)$ marked. The reconstruction Nyquist frequency is 1.28 lp/mm.

**Figure 3.** $f_{\mathrm{sat}}(95\%)$ relative to the reconstruction Nyquist frequency. Left and centre: against dose for each kernel at both magnifications, with bands spanning task diameter, contrast and slice thickness. Right: the distribution over all 540 conditions.

**Figure 4.** $G_{\mathrm{useful}}$ against relative dose with 95% bands, for the standard kernel at the lower of the two task contrasts.

**Figure 5.** Share of $d'^2$ arriving from above the conventional Nyquist frequency of 1.28 lp/mm, over anatomical magnification and viewing distance.

**Figure 6.** Maximum added-band share against task diameter, with the absolute $d'$ of the U-HRCT chain at the reference reading condition on the right axis.
