...
>
> **改訂 v1.1(`docs/IORN-009A_H2_preregistration_v1.1.md`、別コミットで凍結):** C2 の条件軸を閉じた語彙に確定し、`pixel_size`・`displayed_matrix` を追加した。根拠は内在的(H3 が表示画素ピッチを中心機構に置くため、画素サイズを振った研究はモデルの中心機構を直接検証する)。改訂時点で**性能データの抽出・digitize・順位相関の計算は一件も未実施**であることを同文書で宣言している。**主解析は v1.1 プール、併記で v1.0 厳格プール(2軸厳格読み)でも研究内 $\rho$ と判定を報告する**(プールの切り分けは `ptx/external.py` の `pool_partition` が機械的に行う)。両プールで結論が変わらなければ改訂は結論に無害であり、変わる場合は両方を並べて報告する。
